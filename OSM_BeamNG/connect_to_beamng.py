...
def main():
    ...
    location = "Hochschule Neu-Ulm, Neu-Ulm, Bavaria, Germany" #osm location for the project
    center = ox.geocode(location)  

    G = ox.graph_from_point(center, dist=200, network_type="drive", simplify=True) 
    G = ox.project_graph(G) #graph with nodes and edges that act as roads in beamng
    #ox.plot_graph(G)
    print(G)
    ...
    #connection to beamng established
    bng = BeamNGpy(
        "localhost",
        25252,
    ...
    #imu sensor attached to beamng vehicle
